...
class DreamRotaryEmbedding(nn.Module):
    ...
    def forward_with_cache(
        self,
        x: torch.Tensor,
        position_ids: torch.LongTensor,        # [B, L]
        prv_cache_position: torch.LongTensor,  # [B, K]  ← int index
    ):
        """
        index-based dKV-Cache 版本
        - prv_cache_position: 已缓存 token 的 position index
        - 当前 step token = 其余 position
        """
        ...
        cur_mask = torch.ones((B, L), dtype=torch.bool, device=device)
        cur_mask.scatter_(1, prv_cache_position.long(), False)

        q_cos = cos[cur_mask[:, :, None].expand_as(cos)] \
                    .view(B, -1, D)
        q_sin = sin[cur_mask[:, :, None].expand_as(sin)] \
                    .view(B, -1, D)

        return (
            q_cos.to(dtype=x.dtype),
            q_sin.to(dtype=x.dtype),
        ), None
...
def apply_rotary_pos_emb_single(x, cos, sin, position_ids=None, unsqueeze_dim=1):
    """
    单输入版本的 RoPE：给已经预先算好的 (cos, sin) 和某一个张量 x 使用。
    用于 dKV-Cache 中 Q 和 K 长度不一致的场景。
    """
    cos = cos.unsqueeze(unsqueeze_dim)
    sin = sin.unsqueeze(unsqueeze_dim)
    # print(cos.shape)
...
class DreamSdpaAttention(DreamAttention):
    """
    Dream attention module using torch.nn.functional.scaled_dot_product_attention.
    This module inherits from `DreamAttention` as the weights of the module stays untouched.
    The only changes are on the forward pass to adapt to SDPA API + dKV-Cache。
    """
    ...
    def forward(
        ...
        use_cache: bool = False,
        cache_position: Optional[torch.LongTensor] = None,
        prv_cache_position: Optional[torch.BoolTensor] = None,
        ...
        if prv_cache_position is not None:
            ...
            key_states, value_states = past_key_value.update_and_read(
                key_states, value_states, self.layer_idx, cache_kwargs
            )

        else:
            if cache_position.any():
                cache_kwargs = {
                    "cache_position": cache_position,
                    "prv_cache_position": prv_cache_position,
                }
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        if prv_cache_position is None:
            ...
            k_position_embeddings = None
        else:
            position_embeddings, k_position_embeddings = self.rotary_emb.forward_with_cache(hidden_states, position_ids, prv_cache_position)
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    position_embeddings=position_embeddings,
                    prv_cache_position=prv_cache_position,
                    k_position_embeddings=k_position_embeddings,  # 如有需要，你也可以在这里提前算好 K 的 RoPE
                )
